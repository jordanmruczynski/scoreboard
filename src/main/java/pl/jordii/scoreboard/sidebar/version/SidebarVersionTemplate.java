...
public abstract class SidebarVersionTemplate {
     ...
     */
    public abstract void renderSidebar(KelpSidebar<?> sidebar, Player player);

    /**
     * Performs a lazy update on the sidebar. A lazy update does not remove all entries/lines
     * from a scoreboard first, like it is done by {@link #updateSidebar(KelpSidebar, Player)}. It only uses the
     ...
     */
    public abstract void lazyUpdate(KelpSidebar<?> sidebar, Player kelpPlayer);
     ...
     */
    public abstract void updateSidebar(KelpSidebar<?> sidebar, Player player);
